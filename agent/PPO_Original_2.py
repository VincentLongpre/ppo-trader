...
def episode(agent, n_episodes, max_iter=1000, end_update=True):
    ...
    batch_r, batch_s, batch_a = torch.tensor(batch_r, dtype=torch.float), torch.tensor(batch_s,
                                                                                       dtype=torch.float), torch.tensor(
        batch_a, dtype=torch.float)
    if end_update:
...
def hyperparams_run_gradient(agent_class, policy_class_actor, policy_class_critic, env, learning_rates, gamma, clip, ent_coef, critic_factor,
                             max_grad_norm, n_updates, n_episodes, max_iter):
